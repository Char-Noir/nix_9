...
public class PageData<REQ extends ResponseDto> {
    ...
    private static int getLastPage(int pagesize, long itemssize) {
        if (pagesize == 0) {
            throw new RuntimeException("PageSize can not be 0");
        }
        return (int) ((itemssize / pagesize) + (((itemssize % pagesize) > 0) ? 1 : 0));
    }

    public void initPaginationState() {
        if (pageSize < itemsSize) {
            this.totalPageSize = getLastPage(pageSize, itemsSize);
        ...
        currentShowToEntries = Math.min(currentShowFromEntries + pageSize, itemsSize);
    }
